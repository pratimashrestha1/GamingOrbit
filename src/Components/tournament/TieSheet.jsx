...
//     const [error, setError] = useState('');
//     const [players, setPlayers] = useState([]); // Store player data
//     const [byes, setByes] = useState([]); // Players who received byes
//     const location = useLocation();
//     const data = location.state?.data; // Access the passed data
...
//     // New function: Calculate and assign byes
//     const calculateByes = (participants) => {
//         const nextPowerOf2 = Math.pow(2, Math.ceil(Math.log2(participants.length))); // Find the next power of 2
//         const numberOfByes = nextPowerOf2 - participants.length; // Calculate byes
//         const playersWithByes = participants.slice(0, numberOfByes); // Assign byes to the first few players
//         const remainingPlayers = participants.slice(numberOfByes); // Remaining players for matches
//         setByes(playersWithByes); // Store the byes
...
//             .then((response) => response.json())
//             .then((data) => {
//                 const allPlayers = data.participants; // Assuming the API response has a 'participants' key
//                 const remainingPlayers = calculateByes(allPlayers); // Assign byes and return players for matchups
//                 setPlayers(remainingPlayers); // Set remaining players for the matches
//                 setInput(allPlayers.length); // Set input to the total number of players
//             })
//             .catch((err) => console.error('Error loading players:', err));
//     }, [data]);

//     const rounds = generateBrackets(players.length);
            ...
            {visible && (
                <>
                    ...
                    <div className="brackets">
                        {rounds.map((round, roundIndex) => (
                            <div key={roundIndex} className="column">
                                {/* First column: Byes + Matches */}
                                {roundIndex === 0
                                    ? [
                                        // Add byes to the first round column
                                        ...byes.map((bye, i) => (
                                            <input
                                                key={`bye-${i}`}
                                                value={`${bye.username} (Bye)`}
                                                readOnly
                                                className="player-input bye-input"
                                            />
                                        )),
                                        // Add matches to the first round column
                                        ...Array.from({ length: round }, (_, i) => (
                                            <input
                                                key={`${roundIndex}-${i}`}
                                                value={players[i]?.username || ''} // Display player username or leave blank if not available
...
const Div = styled.div`
    ...
    .bye-input {
        background-color: #666; /* Differentiating color for byes */
        font-size: 1.1rem;
    }
