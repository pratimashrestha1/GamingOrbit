...
function InputAndSvg(props) {
  ...
  const downloadAsJpg = () => {
    if (containerRef.current) {
      domtoimage
        .toJpeg(containerRef.current, { quality: 1, bgcolor: "#ffffff" })
        .then((dataUrl) => {
  ...
  return (
    <Container>
      ...
      {isPowerOfTwo(participants.length) ? (
        <div ref={containerRef} className="renderTierContainer">
          {renderTiers(count)}
        </div>
      ) : (
        <div className="error-message">
          <h3>
            The number of participants must be a power of 2 (e.g., 2, 4, 8, 16,
            etc.)
          </h3>
        </div>
      )}
      <button onClick={downloadAsJpg} className="download-btn">
        Download as JPG
      </button>
    </Container>
  );
